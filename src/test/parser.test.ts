...
[
    ...
    {
        expression: '"$table2.column2" EQ false',
        tokenList: [
            { type: 'LITERAL_VALUE', value: '$table2.column2' } as ValueToken, { type: 'EQ' }, { type: 'BOOLEAN', value: false } as ValueToken,
            { type: 'END' },
        ],
        expectedTree: { '$table2.column2' : { [Op.eq] : false } }
    },
    {
...
describe('Parser', () => {
    ...
    beforeAll( () => {
        ...
        parser = new Parser( 
            { 
                primary : (p) => {
                    if(p.lValue.includes('void')) return;
    ...
    test.each( operationsErrorsToTest.map( o => [o.expression, o.tokenList, o.expectedErrors] ))('Expecting errors of %s', 
        async (_, tokenList, expectedErrors) => {
